...
Set `params.protected.enabled = true`, then mark a post with `protected = true` and provide a payload generated by the site-level protection workflow. The theme contains no passwords, plaintext or site-specific encrypted content. Successful unlocks reuse the normal article body, TOC and code-copy behavior. The protected reader ships its own Marked, Highlight.js and DOMPurify browser assets, and sanitizes decrypted Markdown before inserting it into the page, so a standalone theme install does not depend on site-owned vendor files.
